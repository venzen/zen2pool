...
nets = dict(
    bihthai=math.Object(
        ...
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'bihthaiaddress' in (yield bitcoind.rpc_help()) and
        ...
        BLOCK_PERIOD=15, # s
        SYMBOL='BTH',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'Bihthai') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/Bihthai/') if platform.system() == 'Darwin' else os.path.expanduser('~/.bihthai'), 'bihthai.conf'),
